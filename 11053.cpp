...
//BFS Provided by GeeksForGeeks https://www.geeksforgeeks.org/breadth-first-search-or-bfs-for-a-graph/
/*class Graph {
...
bl Graph::checkEdge(ll v, ll s) {
    foreach(x, adj[v]) if (x == s) return 1;
    return 0;
}
//Put into Graph::BFS(ll s) if every function call is exclusive.
vl Graph::BFS(ll s)
{
    vb visitedNode;
    vl res;
    // Mark all the vertices as not visited
    visitedNode.resize(V, 0);
    ...
    while (!queue.empty()) {
        ...
        for (auto& adjacent : adj[s]) {
            if (!visitedNode[adjacent]) {
                visitedNode[adjacent] = 1;
...
void dfs(vl visited, vl2 g, int s) {
    if (visited[s]) return;
    visited[s] = 1;
