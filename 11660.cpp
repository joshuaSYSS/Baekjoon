...
// whitecard.cpp : This file contains the 'main' function. Program execution begins and ends there.
#include <iomanip>
#include <iostream>
#include <cctype>
#include <string>
...
#pragma endregion
ll query(ll block_size, vl blocks, vl v, int r) {
    ll res = 0;
...
ll query(int l, int r, ll block_size, vl blocks, vl v) { return query(block_size, blocks, v, r) - query(block_size, blocks, v, l - 1); }
_m_{ //main関数の開始
...
up(1, n + 1, 1) {
    ...
    up2(1, n + 1, 1) v[i][j] += v[i - 1][j];
}
while(t--){
    ll x1 = get(), y1 = get(), x2 = get(), y2 = get();
