...
str s; cin >> s;
ll idx = -1;
up(0, s.length(), 1) if (s[i] == 'x') idx = i;
if (idx == s.length() - 1) {
    if (idx == 0) cout << "0.5xx+W\n";
    else if (s[0] == '-' && idx == 1) cout << "-0.5xx+W\n";
    else if (stod(s.substr(0, s.length() - 1)) / 2 == 1) cout << "xx+W\n";
    else if (stod(s.substr(0, s.length() - 1)) / 2 == -1) cout << "-xx+W\n";
    else cout << stod(s.substr(0, s.length() - 1)) / 2 << "xx+W\n";
}
else if (idx == -1) {
    if (stod(s) == 0) cout << "W\n";
    else if (stod(s) == 1) cout << "x+W\n";
    else if (stod(s) == -1) cout << "-x+W\n";
    else cout << stod(s) << "x+W\n";
}
else {
    if (idx == 0) cout << "0.5xx";
    else if (stod(s.substr(0, s.length() - 1)) / 2 == 1) cout << "xx";
    else if (stod(s.substr(0, s.length() - 1)) / 2 == -1) cout << "-xx";
    else cout << stod(s.substr(0, s.length() - 1)) / 2 << "xx";
    cout << s[idx + 1];
    s = s.substr(idx + 2);
    if (stod(s) == 1) cout << "x+W\n";
    else cout << stod(s) << "x+W\n";
}
/*
6x+6
