...
int main(void) {
    ...
    vector<vector<char>> chess(8, vector<char>(8)); //ChessBoard
    ll r = 0, c = 0;
    //Convert the ASCII input into a 2D array
    for(ll i = 0;i < 17;i++){
        string input; cin >> input;
        if(i%2){
            for(ll j = 0;j < input.length();j++){
                if((j-2)%4==0) chess[r][c] = input[j], c++;
            }
            r++, c = 0;
        }
    }
    //Handle King and Queen first
    string whiteKing = "K", blackKing = "K", whiteQueen = "Q", blackQueen = "Q";
    for(ll i = 0;i <= 7;i++){
        for(char c = 'a';c <= 'h';c++){
            if(chess[i][c-'a'] == 'K'){
                whiteKing += c;
                whiteKing += to_string(8 - i);
            }
            else if(chess[i][c-'a']=='k'){
                blackKing += c;
                blackKing += to_string(8 - i);
            }
            if(chess[i][c-'a'] == 'Q'){
                whiteQueen += c;
                whiteQueen += to_string(8 - i);
            }
            else if(chess[i][c-'a']=='q'){
                blackQueen += c;
                blackQueen += to_string(8 - i);
            }
        }
    }
    //Handle Rook and Bishop (I forgot Knight so I added it later)
    string wr1 = "R", wr2 = "R", br1 = "R", br2 = "R";
    string wb1 = "B", wb2 = "B", bb1 = "B", bb2 = "B";
    for(ll i = 0;i <= 7;i++){
        for(char c = 'a';c <= 'h';c++){
            if(chess[i][c-'a']=='b'){
                if(bb1 == "B"){
    ...
    //Then check Knight
    string wn1 = "N", wn2 = "N", bn1 = "N", bn2 = "N";
    for(ll i = 0;i <= 7;i++){
        for(char c = 'a';c <= 'h';c++){
            if(chess[i][c-'a']=='n'){
                if(bn1 == "N"){
    ...
    //Last check Pawn
    vector<string> whitePawn, blackPawn;
    for(ll i = 0;i <= 7;i++){
        for(char c = 'a';c <= 'h';c++){
            if(chess[i][c-'a']=='p'){
                string res = "";
    ...
    for(ll i = 7;i >= 0;i--){
        for(char c = 'a';c <= 'h';c++){
            if(chess[i][c-'a']=='P'){
                ...
                res += c;
                res += to_string(8 - i);
                whitePawn.push_back(res);
            }
        }
    }

    //Check existence of chess piece
    string res = "", res2 = "";
    if(whiteKing != "K") res += whiteKing + ",";
    if(blackKing != "K") res2 += blackKing + ",";
    if(whiteQueen != "Q") res += whiteQueen + ",";
    if(blackQueen != "Q") res2 += blackQueen + ",";
    if(wr1 != "R") res += wr1 + ",";
    if(br1 != "R") res2 += br1 + ",";
    if(wr2 != "R") res += wr2 + ",";
    if(br2 != "R") res2 += br2 + ",";
    ...
    foreach(x, whitePawn) res += x + ",";
    foreach(x, blackPawn) res2 += x + ",";
    
    if(res[res.length() - 1] == ',') res = res.substr(0, res.length() - 1);
    if(res2[res2.length() - 1] == ',') res2 = res2.substr(0, res2.length() - 1);
