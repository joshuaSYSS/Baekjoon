...
/*
SOLUTION:
The solution is quite similar to Xenia & Tree (CodeForces).
After we centroid decomposed the tree, we maintain an array of multiset. The multiset will always return the smallest value (distance), so when we query for the distance, the ancestor will store the value.
Theoretically, we don't have to have an array keeping the state of the node, as we can check does the minimum distance of the current node to be done when op = 1 equal to 0. But my initial solution WA-ed, and this solution is AC, so idk.
...
vector<multiset<ll>> minDist(N);
vector<bool> isWhite(N, 0);
tc{
    ll op = get(), idx = get();
idx--;
if (op == 1) {
    if (isWhite[idx]) { //white -> black
        minDist[idx].erase(minDist[idx].find(0));
        isWhite[idx] = 0;
        foreach(x, ancestors[idx]) {
            ll ancestor = x.first, dist = x.second;
            minDist[ancestor].erase(minDist[ancestor].find(dist));
        }
    }
    else { //black -> white
        minDist[idx].insert(0);
        isWhite[idx] = 1;
        foreach(x, ancestors[idx]) {
            ll ancestor = x.first, dist = x.second;
            minDist[ancestor].insert(dist);
        }
    }
}
else {
    ...
    ll res = LL_MAX;
    if (minDist[idx].size()) res = *minDist[idx].begin();
    foreach(x, ancestors[idx]) {
        ll ancestor = x.first, dist = x.second;
        if (minDist[ancestor].size()) res = min(res, *minDist[ancestor].begin() + dist);
    }
    if (res == LL_MAX) res = -1;
    cout << res << '\n';
}
