...
vb r;  // removed
vl s;   // subtree size
vl2 tree, centroidTree;
ll centroid_dfs(ll n, ll p = 0) {
    s[n] = 1;
    for (ll x : tree[n])
        if (x != p && !r[x]) s[n] += centroid_dfs(x, n);
    return s[n];
}
ll get_centroid(ll n, ll ms, ll p = 0)  // n = node, ms = size of tree, p = parent
{
    for (int x : tree[n])
        if (x != p && !r[x])
...
ll centroid_(ll n = 1) {
    ll C = get_centroid(n, centroid_dfs(n));
    r[C] = 1;
    for (ll x : tree[C])
        if (!r[x]) { 
            ll sub = centroid_(x);
            centroidTree[sub].push_back(C), centroidTree[C].push_back(sub);
        }
    return C;
}
ll centroid(ll SIZE) {
...
//8: 2
//
// Prove by AC: (When accepted then this conjecture is proven without any math or rigorous prove)
// Odd = even or odd
// n is divisible by 4 = even
// n is divisible by 2 but not 4 = odd
// 
// Prove by Math:
// Properties of Positive Even and Odd integers (Table 1):
// odd * odd = odd
// odd * even = even * odd = even
// even * even = even
// 
// S(x) = x * (x + 1) / 2
// Now the problem becomes determining S(y) - S(y - n) is even or odd for any positive integer y
// y * (y + 1) / 2 - (y - n) * (y - n + 1) / 2
// ...after a bit of first grade maths...
// ny - (n^2 / 2) + (n / 2)
// 
// When n is divisible by 2, n = 2k
// 2ky - 2k^2 + k
// k * (2y - 2k + 1)
// 
// Notice (2y - 2k + 1) is odd for any y and k
// So now it depends on k since Table 1 told us so.
// 
// Therefore when k is divisible by 2, k = 2q
// 2q * odd = even
// AND since n = 2k and k = 2q, n = 4q. Thus only when n is divisible by 4 you can even.
// 
// In constrast, when k isn't divisible by 2
// k * odd = odd
// 
// Looking back, lets handle when n isn't divisible by 2.
// ny - (n^2 / 2) + (n / 2)
// ny - (odd / 2) + (even + 0.5)
// ny - even - 0.5 + even + 0.5
// ny
// By Table 1, notice when n is odd, ny can be even (y = even) and odd (y = odd). Thus we cannot determine
// 
// -----------------
// And we proved it!
// -----------------
// 
// well after all this math isn't required by prove of accepted but if you don't feel so good then
...
//
// Google Translate:
// AC에 의한 증명: (수락되면 이 추측은 수학이나 엄격한 증명 없이 증명됩니다.)
// 홀수 = 짝수 또는 홀수
// n은 4로 나눌 수 있습니다 = 짝수
// n은 2로 나눌 수 있지만 4는 아님 = 홀수
//
// 수학으로 증명:
// 양의 짝수 및 홀수 정수의 속성(표 1):
// 홀수 * 홀수 = 홀수
// 홀수 * 짝수 = 짝수 * 홀수 = 짝수
// 짝수 * 짝수 = 짝수
//
// S(x) = x * (x + 1) / 2
// 이제 문제는 S(y) - S(y - n)이 양의 정수 y에 대해 짝수인지 홀수인지 결정하는 것입니다.
// y * (y + 1) / 2 - (y - n) * (y - n + 1) / 2
// ...1학년 수학을 조금 마친 후...
// ny - (n^2 / 2) + (n / 2)
//
...
// k * (2y - 2k + 1)
//
// 공지사항 (2y - 2k + 1)은 y와 k에 대해 홀수입니다.
// 이제 표 1에서 알려준 대로 k에 따라 달라집니다.
//
// 따라서 k가 2로 나누어지면 k = 2q입니다.
// 2q * 홀수 = 짝수
// AND n = 2k이고 k = 2q이므로 n = 4q입니다. 따라서 n이 4로 나누어질 때만 짝수가 가능합니다.
//
// 반대로 k가 2로 나누어지지 않는 경우
// k * 홀수 = 홀수
//
// 다시 찾고, n이 2로 나누어지지 않을 때를 처리해 보겠습니다.
// ny - (n^2 / 2) + (n / 2)
// ny - (홀수 / 2) + (짝수 + 0.5)
// ny - 짝수 - 0.5 + 짝수 + 0.5
// 뉴욕
// 표 1에 따르면 n이 홀수일 때 ny는 짝수(y = 짝수)일 수도 있고 홀수(y = 홀수)일 수도 있습니다. 따라서 우리는 결정할 수 없습니다
//
// -----------------
// 그리고 우리는 그것을 증명했습니다!
// -----------------
//
// 결국 이 수학은 증명에 필요하지 않지만 기분이 별로 좋지 않다면
// 여전히 이 작업을 수행할 수 있습니다.
cout << (n % 2 ? 0 : !(n % 4) ? 2 : 1) << '\n';
stop  //編碼程序結束
}
