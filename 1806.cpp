...
vl v(n); vget(v);
vl prefixSum(n + 1, 0);
up(1, n + 1, 1) prefixSum[i] += prefixSum[i - 1] + v[i - 1];
ll minimal = LL_MAX;
if (v[0] == s) {
    cout << 1 << endl;
    stop;
...
up(0, n + 1, 1) {
    ll p = prefixSum[i] + s;
    auto f = lower_bound(all(prefixSum), p);
    if (f != prefixSum.end()) minimal = min(minimal, distance(prefixSum.begin(), f) - i);
}
if (minimal == LL_MAX) cout << 0 << endl;
